    ...
    <div class="navbar-content d-flex align-items-center justify-content-between">
      ...
      <div class="d-flex align-items-center gap-3">
        ...
        <select class="form-select select text-uppercase" @change="setUnit(unit)" v-model="unit" required>
          ...
          <option v-for="(program, i) in $auth.user.Program?.split(',')" :key="i" :value="program" class="text-uppercase">
            {{ program }}
          </option>
        </select>
        ...
        <!-- profile -->
        <div class="profile">
          <img @click="viewProfile()" src="~/assets/img/foto.png" alt="foto profil" class="rounded-circle" ref="profile" />
          <transition name="slide-fade">
