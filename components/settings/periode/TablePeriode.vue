    ...
    <table class="table table-hover table-striped">
      ...
      <tbody>
        <tr v-for="(data, i) in periode" :key="i">
          ...
          <td class="text-end">
            <a href="javascript:;" @click="updateItem(data.SK, data.Status)"><button class="btn" :class="data.Status === 'active' ? 'btn-secondary' : 'btn-primary'
          ">
    ...
    async updateItem(key, status) {
      ...
      if (result.isConfirmed) {
        ...
        await this.$axios.$put(
          `update-database?subject=periode&id=${key.split("#")[0]}&code=${key.split("#")[1]}`,
          data
        );
