...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState("tahfidzAbsensi", ["updateData", "selectedType"]), // Map selectedType
      ...mapState("index", ["label", "semester"]),
    },
    methods: {
      async santriAbsen(event) {
        this.btn = false;
        ...
        const type = this.selectedType.toLowerCase(); // Convert to lowercase ('halaqah' or 'halaqahidhofi')

        // Determine correct key dynamically
        const namahalaqah = type === "halaqah" ? this.updateData.santri.Halaqah : this.updateData.santri.HalaqahIdhofi; // or .Idhofi depending on your payload structure

        try {
          // Switch API to use dynamic type
          const result = await this.$apiSantri.$put(
            `update-absensi-sisalam?sksantri=${skSantri}&type=${type}${time}&thn=${tahun}&smstr=${semester}&program=${program}&subject=${namahalaqah}`,
            data,
          );
