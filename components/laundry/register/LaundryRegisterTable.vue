  ...
  <div>
    <div class="row mb-3">
      <div class="col-12 col-md-6 d-flex flex-column justify-content-center mb-3 mb-md-0">
        ...
        <select v-model="selectedAsrama" class="form-select" style="font-size: 12px; max-width: max-content" @change="getDataByAsrama()">
      ...
      <div class="col-12 col-md-6 d-flex justify-content-end align-items-end">
        <div class="input-group">
          ...
          <input style="font-size: 12px !important" type="text" v-model="search" class="form-control" placeholder="Search" />
          <button style="font-size: 12px !important" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#modalAddDataBag">
            Input Data
          </button>
        </div>
      </div>
        ...
        <tbody>
          <tr v-for="(dataItem, index) in filteredDatas" :key="index">
            ...
            <td class="text-capitalize align-middle nowrap">
              <span
                :class="{
                  'badge text-bg-success': dataItem.Status === 'active',
                  'badge text-bg-danger': dataItem.Status === 'lost',
                  'badge text-bg-warning': dataItem.Status === 'damaged',
                  'badge text-bg-secondary': dataItem.Status !== 'active',
                }">
                {{ dataItem.Status }}
              </span>
            </td>
