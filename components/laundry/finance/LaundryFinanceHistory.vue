        ...
        <div class="card p-3 shadow" style="border-radius: 15px" :class="isLastKey(index) ? 'mb-5' : 'mb-3'">
          <div v-for="(data, i) in group" :key="i">
            <a href="javascript:;" @click="showDelete(data, index)" class="list-transaction-detail d-flex justify-content-between">
              <div class="d-flex align-items-center gap-2" :class="group.length > 1 && i !== group.length - 1 ? 'mb-3' : ''">
                <div
                  style="width: 35px; height: 35px"
                  class="d-flex align-items-center justify-content-center rounded-circle"
                  :class="data.PK === 'credit' ? 'bg-danger' : 'bg-success'">
                  <i class="bi text-white" :class="data.PK === 'credit' ? 'bi-credit-card' : 'bi-wallet'" style="font-size: 14px"></i>
                </div>
                <div class="d-flex flex-column gap-1">
                  <p class="fw-bold text-capitalize" style="font-size: 12px; max-width: 150px; white-space: wrap">{{ data.Name }}</p>
              ...
              <div
                ...
                <p style="font-size: 10px">{{ data.ClaimID }}</p>
              </div>
