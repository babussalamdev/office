...
export default {
  computed: {
    ...mapState('rekaptahsin', ['santri']),
    ...mapGetters('rekaptahsin', ['getStart', 'getEnd']),
    start: {
      get() {
      ...
      set(value) {
        ...
        this.$store.commit('rekaptahsin/setState', obj)
      }
    },
    end: {
      get() {
  ...
  methods: {
    ...mapActions('rekaptahsin', ['changeUnit']),
    juz(value) {
      const juz = value / 20
      return juz
    },
    exportToExcel() {
      ...
      const wb = XLSX.utils.table_to_book(table, { sheet: 'Rekap Tahsin' });
      XLSX.writeFile(wb, `Rekap Tahsin ${halaqah}.xlsx`);
    }
  },
};
</script>
