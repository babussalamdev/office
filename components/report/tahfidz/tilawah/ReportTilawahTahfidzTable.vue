    ...
    <div class="row mb-3">
      <div class="col-12 col-md-6 mb-2 mb-md-0">
        <div class="input-group d-flex align-items-center">
          <span class="input-group-text bg-secondary text-white">{{ santri ? santri.length : 0 }} Santri</span>
        ...
        <tbody>
          <tr v-for="(data, index) in santri" :key="index">
            ...
            <td class="text-capitalize align-middle" style="font-family: 'Noto Kufi Arabic', sans-serif; font-size: 12px; font-weight: 600">
              {{ data.From === "-" ? data.From : `${data.From.ayat.number} : ${data.From.name}` }}
            </td>

            <!-- Kolom Akhir -->
            <td class="text-capitalize align-middle" style="font-family: 'Noto Kufi Arabic', sans-serif; font-size: 12px; font-weight: 600">
              {{ data.To === "-" ? data.To : `${data.To.ayat.number} : ${data.To.name}` }}
            </td>

            <!-- Kolom Baru (Halaman) -->
            <td class="text-capitalize align-middle">{{ data.Page }} Hal</td>

            <!-- Kolom Jumlah (Juz / Khatam) -->
            <td class="text-capitalize align-middle">
              <span v-if="data.khatam > 0">{{ data.khatam }} Khatam dan {{ data.remainingJuz }} Juz</span>
              <span v-else>{{ data.Juz || 0 }} Juz</span>
            </td>
          </tr>
        </tbody>
      </table>
...
<script>
  ...
  export default {
    computed: {
      ...mapState("report/tahfidz/tilawah", ["santri", "listKelas"]),
      ...mapGetters("report/tahfidz/tilawah", ["getSelectedKelas", "getStart", "getEnd"]),
      start: {
        get() {
    ...
    methods: {
      ...mapMutations("report/tahfidz/tilawah", ["setState"]),
      ...mapActions("report/tahfidz/tilawah", ["changeUnit", "getSantri"]),
      ...
      exportToExcel() {
        ...
        const wb = XLSX.utils.table_to_book(table, { sheet: "Tilawah Santri" });
        XLSX.writeFile(wb, `Report Tilawah Tahfidz ${halaqah.Nama}.xlsx`);
      },
    },
  };
</script>
