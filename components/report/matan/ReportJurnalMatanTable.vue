      ...
      <div class="animate__animated animate__fadeInUp">
        <div class="head row mb-3 row">
          <div class="col-12 col-md-6">
            <h2 class="mb-3 mb-md-3">Report Jurnal Matan</h2>
            <div class="d-flex flex-column flex-md-row gap-4 gap-md-0 mb-3 mb-md-0">
              <div class="input-group">
                ...
                <select class="form-select" aria-label="Default select example" v-model="selectedSemester" @change="changeGetMatanSemester">
                ...
                <select
                  ...
                  @change="changeGetMatan">
                  <option value="" selected disabled>Kelas</option>
                  <option v-for="(data, index) in kelas" :key="index" :value="data">{{ data }}</option>
                </select>
                <select class="form-select" aria-label="Default select example" v-model="selectedMatan" @change="addNewData">
                  <option value="" selected disabled>Matan</option>
                  <option v-for="(data, index) in matan" :key="index" :value="data.SK.split('#')[3]">{{ data.Nama }}</option>
                </select>
              </div>
            </div>
          </div>
...
<script>
  ...
  export default {
    computed: {
      ...mapState("report/matan/jurnal", ["matan", "kelas", "label", "semester", "th", "values"]),
      ...mapGetters("report/matan/jurnal", ["getSelectedMatan", "getSelectedKelas", "getSelectedLabel", "getSelectedSemester"]),
      walas() {
      ...
      selectedMatan: {
        ...
        set(value) {
          const obj = { key: "selectedMatan", value };
          this.$store.commit("report/matan/jurnal/setState", obj);
        },
      },
      selectedKelas: {
        get() {
        ...
        set(value) {
          this.$store.commit("report/matan/jurnal/setState", { key: "selectedKelas", value });
        },
      },
      selectedLabel: {
        get() {
        ...
        set(value) {
          this.$store.commit("report/matan/jurnal/setState", { key: "selectedLabel", value });
        },
      },
      selectedSemester: {
        get() {
        ...
        set(value) {
          this.$store.commit("report/matan/jurnal/setState", { key: "selectedSemester", value });
        },
      },
    },
    methods: {
      ...mapActions("report/matan/jurnal", ["getMatan", "changeGetMatanSemester", "changeUnit", "getData"]),
      changeGetMatan() {
        this.getMatan();
      },
      addNewData() {
