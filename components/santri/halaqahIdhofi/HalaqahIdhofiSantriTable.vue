    ...
    <div>
      <div class="row mb-3">
        <div class="col-12 col-md-9 d-flex align-items-center">
          <h2 class="text-capitalize mb-3 mb-md-0">data Idhofi</h2>
        </div>
        <div class="col-12 col-md-3 d-flex justify-content-end">
          <div class="input-group">
            <button type="button" class="btn btn-primary btn-sm" @click="editBulk(data)" :disabled="data.length > 0 ? false : true">Edit</button>
        ...
        <table class="table table-hover table-striped">
          ...
          <tbody>
            <tr v-for="(data, index) in santri" :key="index">
              <td>
                <input class="form-check-input" type="checkbox" v-model="selectedItems[data.SK]" @change="getCheckedNames" />
              </td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getCheckedNames() {
        const checkedNames = Object.keys(this.selectedItems).filter((key) => this.selectedItems[key]);
