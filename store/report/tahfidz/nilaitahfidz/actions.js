...
export default {
  async changeUnit({ commit, state, dispatch, rootState }) {
    ...
    if (result.quran.length > 0) {
      ...
      if (jabatan.includes("koordinator tahfizh")) {
        ...
        if (kelas !== "off") {
          const findKelas = data.find((x) => x.SK.split("#")[3] === kelas);
  ...
  async getSantri({ commit, state, dispatch, rootState }) {
    ...
    const datas = {};
    datas["Filter"] = "report-quran-tahfidz";
    datas["Kelas"] = state.selectedKelas.SK.split("#")[3];
    datas["Subject"] = "quran";
    ...
    try {
      if (status === "active") {
        ...
        datas["TargetPencapaian"] = state.selectedKelas.Pencapaian;
