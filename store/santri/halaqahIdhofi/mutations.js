export default {
  ...
  editBulk(state, value) {
    $("#updateDataSantriHalaqahIdhofi").modal("show");
    state.updateData = value;
  },
  updateHalaqahIdhofiSantri(state, data) {
    const updatedData = state.santri.map((item) => {
      ...
      if (data.sort.includes(item.SK)) {
        ...
        updatedItem["HalaqahIdhofi"] = data.value;
        return updatedItem;
      }
    ...
    state.asramaShow = "";
    $("#updateDataSantriHalaqahIdhofi").modal("hide");
  },
