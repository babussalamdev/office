...
export default {
  ...
  setStatusPengujiUjianJuz(state, data) {
    const i = state.pegawai.findIndex((x) => x.SK === data.index);

    // Check if the property exists on the object, initialize if it doesn't
    if (!state.pegawai[i].Penguji_Ujian_Juz) {
      state.pegawai[i].Penguji_Ujian_Juz = {};
    }

    // Update the specific unit property
    state.pegawai[i].Penguji_Ujian_Juz[data.unit] = data.condition;

    // Reassign array to trigger Vue 2 reactivity for newly added objects
    state.pegawai = [...state.pegawai];
  },
