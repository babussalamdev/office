...
export default {
  ...
  async resetPIN({ commit, state, dispatch }, cnc) {
    ...
    if (result.isConfirmed) {
      dispatch("index/submitLoad", null, { root: true });
      try {
        const datas = await this.$apiCard.$put(`reset-pin?cnc=${cnc}`);
        if (datas) {
          commit("BTN");
          Swal.fire({
            position: "center",
            icon: "success",
            html: `Nomor PIN baru <span style="font-weight:700">${name}</span> adalah : <span style="font-weight:700">${datas.New_PIN}</span>`,
