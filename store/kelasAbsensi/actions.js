...
export default {
  async changeUnit({ commit, dispatch, state, rootState }, data) {
    ...
    const resSelect = await this.$apiBase.$get(`get-settings?type=absensikelas&program=${program}&smstr=${semester}`);
  ...
  async getDataSantri({ commit, dispatch }, data) {
    dispatch("index/submitLoad", null, { root: true });
    const program = localStorage.getItem("program");
    const result = await this.$apiSantri.$get(`get-absensi-sisalam?type=every&subject=kelas&program=${program}&value=${data}`);
