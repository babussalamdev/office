  ...
  async changeUnit({ commit, dispatch }, data) {
    ...
    if (data === 'root') {
      ...
      if (result) {
        ...
        const newHeaders = { Nama: '', Program: '' };

        // Loop melalui setiap item dalam newData
        for (const item of newData) {
          const programName = item.Name; // Ambil nama program
          newHeaders[programName] = ''; // Tambahkan ke newHeaders dengan nilai awal kosong
        }

        // Tambahkan properti 'Action' di akhir
        newHeaders['Action'] = '';
