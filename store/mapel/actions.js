...
export default {
  async changeUnit({ commit }, data) {
    const result = await this.$apiBase.$get(`get-settings?sk=${data}&type=opsimapel`);
    commit("setMapel", result);
  },
  async getKelas({ commit }, data) {
    const result = await this.$apiBase.$get(`get-settings?sk=${data.program}%23${data.kelas}%23${data.semester}&type=mapel`);
  ...
  async inputMapel({ commit, state }, event) {
    ...
    console.log(state.selectSemester);
    ...
    if (state.value.length === 0) {
      ...
      try {
        const result = await this.$axios.$post(
          `input-settings?program=${program}&kls=${state.selectKelas}&type=mapel&smstr=${state.selectSemester}`,
          data,
        );
    ...
    if (state.value.length === 0) {
      ...
      try {
        const result = await this.$axios.$put(
          `update-settings?sk=${key}&program=${program}&kls=${state.selectKelas}&type=mapel&smstr=${state.selectSemester}`,
          data,
        );
        if (result.message === "Kelas terisi dengan mapel lain") {
          Swal.fire({
  ...
  async deleteItem({ commit, state }, key) {
    ...
    if (result.isConfirmed) {
      ...
      const program = localStorage.getItem("program");
      const result = await this.$axios.$delete(`delete-settings?sk=${sk}&type=mapel`);
  ...
  async inputDataPenilaian({ commit, state }, event) {
    ...
    const arrAmount = state.Penilaian[i]
      .map((item) => {
    ...
    try {
      ...
      const result = await this.$apiBase.$put(
        `update-settings?sk=${sk}&program=${program}&kls=${state.selectKelas}&type=mapel&smstr=${state.selectSemester}`,
        data,
      );
      commit("updateScore");
      commit("btn");
  ...
  async delScore({ commit, state }, datas) {
    ...
    try {
      ...
      const result = await this.$apiBase.$put(`update-settings?sk=${sk}&type=mapel&smstr=${state.selectSemester}`, data);
    } catch (error) {
      Swal.fire({
        icon: "warning",
        text: error,
