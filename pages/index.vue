    ...
    <div v-if="unit" class="home animate__animated animate__fadeIn">
      <div v-if="$auth.user.role !== 'root' && unit !== 'sarpras'">
        ...
        <div class="row">
          ...
          <!-- Insert this inside the second column, directly under <Violation /> -->
          <div class="col-12 col-lg-6 mb-3 h-100">
            <!-- pelanggaran -->
            <Violation />
          </div>
          <div class="row" v-if="isKepalaUnit">
            <div class="col-12 mb-3">
              <div class="card p-3 shadow-sm border-settings">
                <div class="d-flex justify-content-between align-items-center flex-wrap">
                  <div class="mb-2 mb-sm-0">
                    <h6 class="mb-1 font-weight-bold report-title">Laporan Tengah Semester</h6>
                    <span class="text-muted report-desc">Aktifkan untuk menampilkan halaman laporan tengah semester.</span>
                  </div>
                  <div class="d-flex align-items-center">
                    <label class="switch mb-0 mr-3">
                      <input type="checkbox" v-model="reportStatusLocal" @change="toggleStatus" />
                      <span class="slider round"></span>
                    </label>
                    <span
                      class="font-weight-bold"
                      :class="reportStatusLocal ? 'text-success' : 'text-secondary'"
                      style="min-width: 25px; margin-left: 6px">
                      <!-- Adjust 12px to your liking -->
                      {{ reportStatusLocal ? "On" : "Off" }}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div class="reload-button">
          <button class="Btn" @click="reloadData">
            <svg
              ...
              <path
                style="fill: #344e5d"
                d="M473.677,236.842c0-130.814-106.036-236.838-236.835-236.838C106.036,0.004,0,106.032,0,236.842
              c0,130.795,106.036,236.831,236.842,236.831C367.641,473.673,473.677,367.637,473.677,236.842z" />
              <g>
                <path
                  style="fill: #ffffff"
                  d="M414.449,297.547c-17.243-27.052-34.494-54.108-51.741-81.156c-2.909-4.57-9.218-4.458-12.12,0.045
                ...
                <path
                  style="fill: #ffffff"
                  d="M370.457,138.899c-33.97-48.633-89.75-76.478-149.366-69.65
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState("index", ["unit"]),
      ...mapState("home", ["reportStatus"]), // Ensure this maps to your actual Vuex module name
      isKepalaUnit() {
        if (!process.client) return false; // Prevent Nuxt SSR localStorage error
        const program = localStorage.getItem("program");
        const allowedRoles = ["kepala unit smai", "kepala unit smpi"];
        // console.log(this.$auth.user?.Jabatan?.[program]);
        return allowedRoles.includes(this.$auth.user?.Jabatan?.[program]);
      },
      reportStatusLocal: {
        get() {
          return this.reportStatus === "on";
        },
        set(value) {
          // Handled by the toggleStatus method
        },
      },
    },
    ...
    methods: {
      ...
      async toggleStatus(event) {
        ...
        await this.$store.dispatch("home/updateLaporanStatus", newStatus);
      },
    },
  };
</script>
...
<style scoped>
  ...
  input:checked + .slider {
    background-color: #344e5d; /* Matches dark theme color */
  }
